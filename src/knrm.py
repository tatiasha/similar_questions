...
class KNRM(torch.nn.Module):
    ...
    def _get_kernels_layers(self) -> torch.nn.ModuleList:
        ...
        mus = [1] + [round(1 - (2 * i + 1) / (self.kernel_num  - 1), 2) for i in range(self.kernel_num -1)]
        mus.sort()
        
        sigmas = [self.sigma] * (self.kernel_num - 1) + [self.exact_sigma]  
        
        for m, s in zip(mus, sigmas):
            kernels.append(GaussianKernel(mu=m, sigma=s))
        return kernels

    def _get_mlp(self) -> torch.nn.Sequential:
        model = []
        prev_shape = self.kernel_num
        for idx, i in enumerate(self.out_layers):
            model.append(torch.nn.ReLU())
            model.append(torch.nn.Linear(prev_shape, i))
            prev_shape = i
        
        if len(self.out_layers) != 0:
            model.append(torch.nn.ReLU())
        
        model.append(torch.nn.Linear(prev_shape, 1))
    ...
    def _get_matching_matrix(self, query: torch.Tensor, doc: torch.Tensor) -> torch.FloatTensor:
        query_emb = self.embeddings(query).unsqueeze(2)
        doc_emb = self.embeddings(doc).unsqueeze(1)
    ...
    def predict(self, inputs: Dict[str, torch.Tensor]) -> torch.FloatTensor:
        ...
        print('kernels_out', kernels_out.shape)
        # shape = [Batch]
...
class RankingDataset(torch.utils.data.Dataset):
    ...
    def _convert_text_idx_to_token_idxs(self, idx: int) -> List[int]:
        tokenized_text = self.idx_to_text_mapping.get(str(idx))
        proc_tokenized_text = self.preproc_func(tokenized_text)
        return self._tokenized_text_to_index(proc_tokenized_text)
...
class TrainTripletsDataset(RankingDataset):
    def __getitem__(self, idx):
        query, left_doc, right_doc, rel = self.index_pairs_or_triplets[idx]
        query_token = self._convert_text_idx_to_token_idxs(query)
        left_token = self._convert_text_idx_to_token_idxs(left_doc)
        right_token = self._convert_text_idx_to_token_idxs(right_doc)
        return {'query': query_token, 'document': left_token}, {'query': query_token, 'document': right_token}, rel


class ValPairsDataset(RankingDataset):
    def __getitem__(self, idx):
        query, doc, rel = self.index_pairs_or_triplets[idx]
        query_token = self._convert_text_idx_to_token_idxs(query)
        doc_token = self._convert_text_idx_to_token_idxs(doc)
        return {'query': query_token, 'document': doc_token}, rel
...
class Solution_KNRM:
    ...
    def simple_preproc(self, inp_str: str) -> List[str]:
        processed_str = self.hadle_punctuation(inp_str.lower())
        return nltk.word_tokenize(processed_str)
    ...
    def get_all_tokens(self, list_of_df: List[pd.DataFrame], min_occurancies: int) -> List[str]:
        ...
        raw_str = set(all_data.text_left.tolist() + all_data.text_right.tolist())
        raw_str = ' '.join(raw_str)
        tokens_str = self.simple_preproc(raw_str)
        tokens_dict = Counter(tokens_str)
        tokens_dict_filter = self._filter_rare_words(tokens_dict, min_occurancies)
        return list(tokens_dict_filter.keys())
    ...
    def create_glove_emb_from_file(self, file_path: str, inner_keys: List[str],
        ...
        vocab_emb = self._read_glove_embeddings(file_path)
        unk_words = list(set(inner_keys) - set(vocab_emb.keys()))
        vocab_words = list(set(inner_keys) & set(vocab_emb.keys()))
        special_w = ['PAD', 'OOV']            
        vocab_list = special_w + vocab_words + unk_words
        
        
        vocab = {}
        emb_matrix = np.zeros((len(vocab_list), 50))
        for idx, w in enumerate(vocab_list):
            vocab[w] = idx
            emb_w = vocab_emb.get(w, None)
            if emb_w is None:
                if w == 'PAD' or w == 'OOV':
                    emb_w = np.zeros(50)
                else:
                    emb_w = np.random.uniform(low=-rand_uni_bound, high=rand_uni_bound, size=50)
            emb_matrix[idx] = emb_w
    ...
    def sample_data_for_train_iter(self, inp_df: pd.DataFrame, seed: int
        ...
        left_id_size_th = inf_df_group_sizes[inf_df_group_sizes > 1]
        groups = inp_df_select[inp_df_select.id_left.isin(left_id_size_th.index)].groupby('id_left')
        ...
        for id_left, group in groups:
            ...
            for ones in ones_ids:
                for zeros in zeroes_ids:
                    r = np.random.rand()
                    if r >= 0.5:
                        out_pairs.append([id_left, ones, zeros, 1])
                    else:
                        out_pairs.append([id_left, zeros, ones, 0])
    ...
    def _dcg(self, ys_true: np.array, ys_pred: np.array, dcg_top_k: int) -> float:
    ...
    def train(self, n_epochs: int):
        ...
        for epoch in range(n_epochs):
            print(epoch)
            cur_loss = 0
            if epoch == 0: #epoch % self.change_train_loader_ep == 0:
                train_dataset = TrainTripletsDataset(self.sample_data_for_train_iter(self.glue_train_df, 0),
            ...
            for batch in train_dataloader:
                left_doc, right_doc, rel = batch
                batch_pred = self.model(left_doc, right_doc)
                loss = criterion(batch_pred, rel)
                loss.backward()
                opt.step()
                cur_loss += loss.item()
